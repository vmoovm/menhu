...
/*				$dede_addonfields .= '<b>'.$ctag->GetAtt('itemname').'：</b>';*/
				switch ($type) {
					case 1:
						$dede_addonfields .= (preg_match("/&".$ctag->GetName()."=/is",$filterarr,$regm) ? '<a title="全部" href="'.str_replace("&".$ctag->GetName()."=".$fields_value,"",$filterarr).'">全部</a>' : '<span class="on">全部</span>').'&nbsp;';
						...
						for ($i=0; $i<count($addonfields_items); $i++)
						{
							...
							$dede_addonfields .= ($fields_value!=urlencode($addonfields_items[$i]) ? '<a title="'.$addonfields_items[$i].'" href="'.$href.'">'.$addonfields_items[$i].'</a>' : '<span class="on">'.$addonfields_items[$i].'</span>')."&nbsp;";
						}
						$dede_addonfields .= '<br/>';
					break;
