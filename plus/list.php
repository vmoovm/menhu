...
if(isset($_GET['ajax'])){
	...
	$pagesize = isset($_GET['pagesize']) ? intval($_GET['pagesize']): 16;//每页多少条，也就是一次加载多少条数据
	...
	$load_num =0;
	if(is_array($temp)){
		$load_num= round(($temp['num']-16)/$pagesize);//要加载的次数,因为默认已经加载了
		$total = $temp['num'];
	}
...
if($tinfos['issystem']==-1)
{
    ...
    if(isset($lv->Fields['corank']) && $lv->Fields['corank'] > 0)
    {
        ...
        if( $cfg_ml->M_Rank < $lv->Fields['corank'] )
        {
            ...
            include_once(DEDETEMPLATE.'/mokuai/view_msg_catalog.htm');
            exit();
        }
    }
}
