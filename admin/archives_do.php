...
else if($dopost=="uploadLitpic")
{
    ...
    else
    {
         ...
         else
         {
               $msg = "<script language='javascript'>
                    parent.document.getElementById('uploadwait').style.display = 'none';
                    window.open('imagecut.php?f=picname&isupload=yes&file={$upfile}', 'popUpImagesWin', 'scrollbars=yes,resizable=yes,statebar=no,width=1100,height=820,left=150, top=50');
                </script>";
         }
    }
    echo $msg;
